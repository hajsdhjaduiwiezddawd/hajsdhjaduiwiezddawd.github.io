/* By Adem KOUKI */
/* GitHub : https://github.com/yustinbieber */
...
helptext += "reset\t\t\t\t\tReset cmd (Remove saved parameters)" + "\n";
...
function create_new_empty_line() {
    ...
    $(".console").append(`<div class="line"><span style="color: ${TEXTCOLOR}" class="path">cmd:~ ${USERNAME}$ </span><input style="color: ${TEXTCOLOR}; background-color: ${BGCOLOR}" class="cmdinput" value=""/></div>`)
    $("input").last().focus();
}
...
function welcomeMsg() {
    ...
    if (curHr < 12) {
        document.getElementById('welcome').innerHTML = `☀️ Buen día ${USERNAME}.`;
    } else if (curHr < 18) {
        document.getElementById('welcome').innerHTML = `🌤️ Buenas tardes ${USERNAME}.`;
    } else {
        document.getElementById('welcome').innerHTML = `🌙 Buenas noches ${USERNAME}.`;
    }
}
